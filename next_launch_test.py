...
import re
from sqlalchemy import Table, Column, String, Integer, Boolean, MetaData, create_engine
import asyncio

engine = create_engine('sqlite:///nextlaunch.sqlite')
metadata = MetaData()
...
def sendLaunch_cloudbot(bot, async_call, db, lchout):
...
def async_call(function, *args, **kwargs):
  ...
  if kwargs:
    #result = yield from loop.run_in_executor(executor, function, *args)
    result = function(*args)
  else:
    #result = yield from loop.run_in_executor(executor, lambda: function(*args, **kwargs))
...
async def main():
   ...
   await ll_api()
   await load_cache(async_call, connection)
   await nextlaunch("", print)
